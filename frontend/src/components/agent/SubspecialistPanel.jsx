  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <Card className="border-3 border-red-300 dark:border-red-700 shadow-xl bg-white dark:bg-slate-900">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-red-800 dark:text-red-200 text-lg font-black">
          ...
          <div className="space-y-3">
            {doctorMatches && doctorMatches.slice(0, 3).map((doctor, index) => (
              <motion.div
                ...
                animate={{ opacity: 1, x: 0 }}
                transition={{ duration: 0.4, delay: index * 0.05 }}
                className="p-3 bg-rose-100 dark:bg-rose-900/30 rounded-xl border-2 border-rose-300 dark:border-rose-700"
              >
                <div className="flex items-start justify-between mb-1">
                  ...
                  <Badge className="bg-red-200 dark:bg-rose-900 text-red-900 dark:text-rose-100 border-2 border-red-400 dark:border-rose-600 font-bold text-xs">
                    {doctor.availability}
                  </Badge>
                </div>
                {doctor.credentials && (
